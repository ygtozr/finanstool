...
const authLoginApi=fs.readFileSync(path.join(root,'api','auth-login.js'),'utf8');
const authRegisterApi=fs.readFileSync(path.join(root,'api','auth-register.js'),'utf8');
const adminInvitesApi=fs.readFileSync(path.join(root,'api','admin-invites.js'),'utf8');
...
assert.match(html,/<title>Özer Finans v7\.2 Önizleme<\/title>/,'Tarayıcı başlığı aday sürüm adını kullanmalı');
...
assert.match(authAccount,/credentials: 'same-origin'[\s\S]*\/api\/auth-login[\s\S]*\/api\/auth-register/,'İstemci güvenli çerezle giriş ve davetli kayıt API’lerini kullanmalı');
...
console.log('Özer Finans v7.2 Upstash üyelik önizleme regresyon testleri başarılı.');
