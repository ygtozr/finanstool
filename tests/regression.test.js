...
assert.match(html,/previousPositions=new Map[\s\S]*playFavoriteMotion\(candidate,[\s\S]*duration:180/,'Favoriler yer değiştirirken komşu kartlar FLIP animasyonuyla akıcı yer açmalı');
assert.match(html,/pending\.ghost\.style\.transform=`translate3d[\s\S]*playFavoriteMotion\(ghost,[\s\S]*duration:190/,'Sürüklenen kart parmağı canlı takip edip bırakılınca hedefe oturmalı');
