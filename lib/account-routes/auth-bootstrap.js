...
const { redis, normalizeEmail, validEmail, tokenHash, hashPassword, saveUser, createSession, sameOrigin, noStore, publicUser } = require('../auth-store');

function safeTokenEqual(input, expected) {
  const left = Buffer.from(tokenHash(String(input || '').trim().toLocaleLowerCase('tr-TR')));
  const right = Buffer.from(tokenHash(String(expected || '').trim().toLocaleLowerCase('tr-TR')));
